...
echo Run MD
mpirun /software/namd/2.9-intel/bin/namd2 MD.NAMD.{sysname}.MD.now.conf > {sysname}.run1.MD002.logout

echo Append .xst information to total run
tail -n+3 {sysname}.run1.MD002.xst >> {sysname}.run1.allMD.xst

echo Strip water from .dcd
/home/blake/bin/catdcd -o {sysname}.run1.MD002.nowat.dcd -i {sysname}.nowat.ind {sysname}.run1.MD002.dcd

echo Create allMD.nowat.dcd
mv {sysname}.run1.allMD.nowat.dcd tmp.dcd
/home/blake/bin/catdcd -o {sysname}.run1.allMD.nowat.dcd tmp.dcd {sysname}.run1.MD002.nowat.dcd
rm tmp.dcd
...
# -- 5. Run next job.
echo Run next 50ns
/usr/local/bin/qsub MD.Orchard.{sysname}.run1.MD003.sh
